...
const ContactPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 2. Abre WhatsApp IMEDIATAMENTE (tem que ser síncrono aqui, senão o navegador
    //    pode bloquear o popup por não estar mais dentro do gesto do clique)
    window.open(whatsappUrl, '_blank', 'noopener,noreferrer');

    // 3. Webhook em background.
    //    IMPORTANTE: antes tinha um fetch pro ipify.org (até 3s) ANTES de mandar
    //    pro webhook. No mobile, abrir o WhatsApp costuma tirar o foco/descarregar
    //    a aba, e nesses 3s de espera o fetch pro webhook muitas vezes nunca
    //    chegava a ser disparado — por isso o Lead sumia. Removido: o n8n já pega
    //    o IP real direto do header da requisição (x-real-ip), não precisa vir do
    //    navegador. Isso deixa o caminho crítico bem mais curto (só o hash, ~ms).
    (async () => {
      ...
      // sendBeacon é feito exatamente pra isso: sobrevive à navegação/fechamento
      // da aba, diferente do fetch normal. Usa ele primeiro; se não tiver
      // suporte, cai pro fetch com keepalive como fallback.
      let sent = false;
